  				...
  				if(dy>0){
  					 ...
  					 if(special.kind==1){
  						 if(special.type==1){
    						...
    						localStorage.setItem("c_no", gid );		
    	  		  			document.location.href=getRootPath()+"/jsp/wx/course/af_little_special.html?v=4";	 
    					 }
				...
				$("#yg_link").html("<p>"+y_n+"</p>");
	  			...
	  			if(free_video.length > 0){
	  				...
	  				for(var i=0;i<free_video.length;i++){
	  					...
	  					if(c_name.length > 16){
	  						c_name = c_name.substring(0,14)+""; 
	  					} 
	  					
	  					var zzz =i+1;
	  					
	  					var v = "<div   class='couponone img"+zzz+"' w='"+free_video[i].watch+"' cno='"+free_video[i].c_no+"'  style='width:30%;float: left;display:inline-block;' onclick='goin_curser(this);' >"
		...
		success: function(data){
			...
			if(s_list.length > 0){
				...
				for(var i=0; i< sc; i++){
					...
					var h1 = "<div class='hotone img"+n+"' style='width:30%;' onclick='into_s(this);' sid='"+s_list[i].special_no+"' skind='"+s_list[i].kind+"' stype='"+s_list[i].type+"' >"
	...
	var special_no=$(obj).attr("sid");
	 ...
	 if(kind==1){
		 	if(type==1){
		 		...
		 		window.location.href=getRootPath()+'/jsp/wx/course/af_special_buy_voice.html';
		 	}
			
	 }else{
		 
		 window.location.href=getRootPath()+'/jsp/wx/course/af_little_special.html?v=3l';
	 }
...
function go_hot(){
	document.location.href=getRootPath()+'/jsp/wx/Guyang/hot_list.html?v=1';
	  			...
	  			if(list!=null){
	  				...
	  				if(list.length>0){
	  					...
	  					var tett=list[i].text;
	  						...
	  						var  is_wc=	list[i].is_wc;
	  							var if_wc="";	
	  							
	  							if(is_wc!=null && is_wc!=""){
	  								
	  									if(is_wc==1){
	  										if_wc="<p class='goods-title1 jie' style='margin-top:10px;font-size:13px;color:#888;border-radius:14px;line-height:16px;border:1px solid #dcdcdc;border-radius:20px;margin-left:120px;margin-right:40px;line-height:22px;padding-left:10px;'>共"+list[i].url.length+"集，已更新完毕</p>";
	  										
	  									}else{
	  										
	  										if_wc="<p class='goods-title1 jie' style='margin-top:10px;font-size:13px;color:#888;border-radius:14px;line-height:16px;border:1px solid #dcdcdc;border-radius:20px;margin-left:120px;margin-right:40px;line-height:22px;padding-left:10px;'>已更新"+list[i].url.length+"集，持续更新中...</p>";	
	  									}
	  		...
	  		success: function(data){ 
	  			...
	  			if(list!=null){
	  				...
	  				for(var  i = 0;i<list.length;i++){
	  						...
	  						if(ssn.length > 20){
	  							ssn = ssn.substring(0,19)+"...";
	  		  				}
	  						if(ttr.length > 35){
	  							ttr = ttr.substring(0,34)+"...";
	  		  				}
	  						if(special!=null){
	  					...
	  					html+="<li class='goods-card card small-pic'>"
	  						...
	  						+"<div class='info  btn1 info-price info-no-price'>"
	  						+" "+cc+" "
	  						+"<p class='goods-title' style='margin-top:14px;'>"+ttr+"</p>"
	  						+"<p class='goods-title weizhujiang' style='margin-top:30px;'>主讲:"+list[i].teacher+"</p>"
