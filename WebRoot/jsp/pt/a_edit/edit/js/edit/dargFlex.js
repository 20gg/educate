	...
	$.fn.dargFlex = function  (classN,json) {
		...
		this.bind('mousedown',function (ev){
			...
			if ($(oEe.target).hasClass(classN)){		//默认以类名匹配拖拽元素
				...
				var dragobj=$(oEe.target).closest(".drag");
				
				ED.dragModel=ED.data.models[dragobj.index()];
				
				ED.data.models.splice(dragobj.index(), 1);
				
				var dragdom=dragobj[0];
				
				var oldY = oEe.clientY;
				var oldX = oEe.clientX;
				var iTop = dragdom.offsetTop;
				var iLeft = dragdom.offsetLeft;
				var iW  = dragdom.offsetWidth;
				var iH = dragdom.offsetHeight;
				...
				$this[0].insertBefore(oSpan,dragdom);	
				
				function move (ev) {
					var oEe = ev || event;
					var newY = oEe.clientY;
					var iNowT = dragdom.offsetTop;
					var iT = newY - oldY + iTop;
					
					$(dragdom).css('top',iT+'px');
				...
				function dragUp (ev) {
					...
					ED.data.models.splice(dragobj.index(), 0, ED.dragModel);
