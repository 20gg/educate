...
<html>
  ...
  <body>
     ...
     <div  style="width: 94%; margin-left: 3%;text-align: right; margin-top: 20px; margin-bottom: 10px;">
  			 ...
  			 <a  class="btn btn-primary"   onclick="insert_bb();">+新增专题</a>
  	 </div> 
    
    <div class="input-group"  style="width: 600px; position: absolute;top:9%;left:3%;"> 
  	 		<span class="input-group-btn form-control" style="width: 80px;">专题名称 </span>  
  	...
  	border: 1px solid #eee;  border-left-width: 5px; border-radius: 3px;border-left-color: #4CAF50; text-align: center;">
  		<tr style="background-color: #fcfcfc;">
    		...
    		<td height="30">
    					...
    					<a href="<%=basePath %>course/update_sp.do?special_no=${special.special_no}" >修改</a>
    					<a href="<%=basePath %>course/specialinfo.do?special_no=${special.special_no}" >详情</a>
    						<a href="javascript: to_have({c_no:'${special.special_no}'})" >专题详情地址</a>
    					</td>	  			  
    				</tr>
...
<div class="modal fade" id="updaterole" tabindex="-1" role="dialog" 
  ...
  function last_page(){
		...
		if(this_num <= 1){
			...
			var page = this_num -1;
			var datas = {"page":page};			
			var url = getRootPath()+"/course/show_special.do";
		    jspost(url, datas); 
		}
	}
function next_page(){
...
function blacklist2(special_no,state) {
  	...
  	$.ajax({
  		type:"post",
  		url: getRootPath() +'/course/special_freeze.do',
  		dataType:"json",
	...
	function deleterole(){
		...
		$.ajax({
			type: "POST",
			url: getRootPath() +'/course/delete_special_nom.do',
...
function  insert_bb(){
	...
	window.location.href=getRootPath()+"/jsp/pt/course/insert_special.jsp";		
	
	
}

function get_data(url_data) {
	jspost(pt_path +"/course/show_special.do", url_data);
}
